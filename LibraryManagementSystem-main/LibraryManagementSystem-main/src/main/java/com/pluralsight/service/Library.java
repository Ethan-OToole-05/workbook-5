...
public class Library {
    ...
    public List<Movie> searchMoviesByDirector(String director) {
        ...
        for (Item item : items.values()) {
            if (item.getCreator().toLowerCase().equalsIgnoreCase(director)) {
                moviesByDirector.add((Movie) item);
            }
        }
        logger.info("Director search for '" + director + "', results: " + moviesByDirector.size());
    ...
    public List<Magazine> searchMagazinesByPublisher(String publisher) {
        List<Magazine> magazinesByPublisher = new ArrayList<>();
        publisher = publisher.toLowerCase();

        for (Item item : items.values()) {
            if (item.getCreator().toLowerCase().equalsIgnoreCase(publisher)) {
                magazinesByPublisher.add((Magazine) item);
            }
        }
        logger.info("Publisher search for '" + publisher + "', results: " + magazinesByPublisher.size());
        return magazinesByPublisher;
    }
